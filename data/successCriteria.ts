...
(() => {
	...
	let addedInTwo = (() => {
		let items = {};
		let section = document.getElementById('new-features-in-wcag-2-2');
		let listitems = section.querySelectorAll('li');
		for (let li of listitems) {
			let num = li.textContent.match(/(\d+)\.(\d+)\.(\d+)/gi)[0];
			items[num] = { versionAdded: '2.2' };
		}
		return items;
	})();
    ...
    for (let principle of principles) {
        ...
        for (let link of [...scLinks]) {
            ...
            try {
				...
				if (addedInTwo[scDesc.number]) scDesc.versionAdded = addedInTwo[scDesc.number].versionAdded; 
